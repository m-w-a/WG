...
TEST(wg_lclcontext_extant_nested, CompletedScope)
{
...
TEST(wg_lclcontext_extant_nested, PartiallyCompletedScope)
{
  RecordKeeper records;
...
TEST(wg_lclcontext_extant_nested, IncompletedScope)
{
  RecordKeeper records;
    ...
    on_exit(scpmngr.exit(scope_completed);) )
  {
      ...
      with(SimpleScopeMngr(ScopeManager::Id2, records)) )
    {
      ...
      WG_TEST_ASSERT_ISNOTREFERENCE(entry1, scpmngr1.EntryObj);
      WG_TEST_ASSERT_ISNOTCONST(entry1);
      WG_TEST_ASSERT_ISSAMETYPE_MODULOCONSTANDREF(EmptyCntr, entry1);
