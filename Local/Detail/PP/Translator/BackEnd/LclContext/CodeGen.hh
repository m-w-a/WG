...
#define WG_PP_LCLCONTEXT_CODEGEN_END(noex) \
  WG_PP_LCLCONTEXT_CODEGEN_END_IMPL(noex)
...
#define WG_PP_LCLCONTEXT_CG_TRYBLOCK(symbtbl) \
  BOOST_PP_IIF( \
    WG_PP_LCLCONTEXT_SYMBOLTABLE_ISNOEX(symbtbl), \
    BOOST_PP_EMPTY, \
    try BOOST_PP_EMPTY) ()
...
#define WG_PP_LCLCONTEXT_CODEGEN_END_IMPL(noex) \
    } \
    \
    BOOST_PP_IIF( \
      noex, \
